...
const uint8_t PROGMEM font5x7 [][CHAR_WIDTH - 1] = {
	...
	{ 0x1C, 0x1C, 0x3E, 0x7F, 0x00 },   //  speaker on 0x8E
	{ 0x1A, 0x14, 0x2E, 0x5F, 0x20 },    //  speaker off 0x8F				
	...
	{ 0x04, 0x26, 0x38, 0x26, 0x04 },	// radsign 0x97
//	{ 0x67, 0x15, 0x60, 0x17, 0x63 }	// CPM sign 0x98
...
LCD_5110::LCD_5110(pin *rst, pin *ce, pin *dc, pin *data, pin *clk, pin *backlight) {
	lcdCacheIdx = 0;
	m_rst = rst;
	m_ce = ce;
	m_dc = dc;
...
//Put display in power down
void LCD_5110::sleep(void) {
send(LCD_SLEEP_MODE,LCD_CMD);
}
//put display in power on mode
void LCD_5110::wake(void) {
send(LCD_WAKE_MODE,LCD_CMD);
}
...
void LCD_5110::clear_area(uint8_t line, uint8_t startX, uint8_t endX) {
    ...
    uint8_t start = line * LCD_WIDTH + startX;
    uint8_t end = line * LCD_WIDTH + endX;
	
	base_addr(start);
    
    // Clear all data in range from cache
    for(uint8_t i = start; i < end; i++) send(0, LCD_DATA);
}
...
void LCD_5110::printPictureOnLCD ( const uint8_t *data) {
	uint16_t pixel_cols = LCD_WIDTH * (LCD_HEIGHT / CHAR_HEIGHT); //<6> means 6 lines on LCD.
 	goto_xy(0, 0);
	for(uint16_t i=0;i<pixel_cols;i++) col(pgm_read_byte(data++));
}
...
bool LCD_5110::getBacklight(void) {
	return *m_backlight;
}
